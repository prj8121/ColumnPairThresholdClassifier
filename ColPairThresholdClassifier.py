...
class ColPairThresholdClassifier(BaseEstimator, ClassifierMixin):
    ...
    def load_data(self, df, target, timer=False):
        ...
        setattr(self, 'class_names', df[target].unique())  # Unique values represented in the target column
        ...
        for col_1 in range(len(self.data_cols)):
            for col_2 in range(col_1, len(self.data_cols)):
                start_time = time.time()
                lines = get_best_lines(df[self.data_cols[col_1]], df[self.data_cols[col_2]], df[target], self.class_names)
